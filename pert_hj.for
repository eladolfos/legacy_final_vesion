CCPY Oct 04, 2008, Add the PERT part for the processes Z0_RA_UUB and Z0_RA_DDB 
...
C------------
     ...
     >   TYPE_V.EQ.'ZG' .OR. TYPE_V.EQ.'GG') THEN ! Initial state is g g
      ...
      ELSE IF(TYPE_V.EQ.'HJ') THEN
       ptj=vmas
C       Q_V=sqrt(mh**2+2d0*ptj*sqrt(mh**2+ptj**2+vpt**2+2d0*vpt*ptj)
C     >    *cosh(rapin)
C     >    +2d0*ptj**2+2d0*ptj*vpt)
      Q_V=dsqrt(mh**2+2d0*ptj**2+2d0*ptj*
     >    dsqrt(mh**2+ptj**2)*dcosh(rapin))
       S_Q=Q_V
      S_LAM=ALAMBD(S_Q)
      NF_EFF=NFL(S_Q)
       CALL HJSETUP(NF_EFF)
...
CSM
        ...
        ELSEIF(TYPE_V.EQ.'HJ')THEN
            CONST = Pi/64d0*(ALPI(S_Q)/(3d0*246d0))**2
        ELSE
          PRINT*,' WRONG TYPE_V'
          CALL QUIT
        ENDIF
...
CsB___Definition of momentum fractions x1 and x2
      IF(TYPE_V.EQ.'HJ') THEN
        X_A=Dexp(-2d0-y_v/2d0)*(-1d0+dexp(4d0))*(ptj+dexp(y_v)
     >     *dsqrt(mh**2+ptj**2))/(4d0*dsqrt(2d0)*ECM)
        X_B=Dexp(-2d0-y_v/2d0)*(-1d0+dexp(4d0))*(dexp(y_v)*ptj+
     >     dsqrt(mh**2+ptj**2))/(4d0*dsqrt(2d0)*ECM)
      Else If (KinCorr.Eq.0) then
...
C  Type_V.Eq.'Z0_RA_UUB' .or. Type_V.Eq.'Z0_RA_DDB'
      ...
      IF(X1LOW.GT.1.0) THEN
       WRITE(NWRT,*)' DOX1=FALSE'
       DOX1=.FALSE.
      ENDIF
      IF(X2LOW.GT.1.0) THEN
       WRITE(NWRT,*)' DOX2=FALSE'
       DOX2=.FALSE.
      ENDIF
...
C THIS IS d(SIGMA)/d(Q^2)d(QT^2)d(Y)
      PERT=HBARC2*ALPI(AMU)*PI*CONST*(X1INTEG+X2INTEG)
...
C --------------------------------------------------------------------------
      SUBROUTINE X1X2INT_PP(X1,X2,SH,TH,UH,AMU,PLUMI)
C --------------------------------------------------------------------------
C
...
C FOR ASYMPTOTIC PART
       IF(IPDF.EQ.0) THEN
          ...
          ELSE
           FPDF=APDF(IPDF,X,AMU)
          ENDIF
         SPLITSUD=(HJSPLIT(IPDF,X,AMU)*X+A_SUD*FPDF)*A_ALPI
       ENDIF
      ENDIF
...
CZL: the prime splitting function convoluting with PDF, ie PxF
...
c      ENDIF
     ...
     .     TYPE_V.EQ.'ZG' .Or. TYPE_V.EQ.'GG' ) Then
      ...
      ELSE IF(TYPE_V.EQ.'HJ') THEN
       ptj=vmas
C       Q_V=dsqrt((2d0*(mh**2+ptj**2)**(3d0/2d0)*(mh**2+2d0*ptj**2)
C     >    +ptj*(4d0*(mh**2+ptj**2)**2+(2d0*mh**2+ptj**2)*vpt**2)
C     >    *cosh(rapin))/(2d0*(mh**2+ptj**2)**(3d0/2d0)))
C       Q_V=sqrt(mh**2+2d0*ptj*sqrt(mh**2+ptj**2+vpt**2+2d0*vpt*ptj)
C     >    *cosh(rapin)
C     >    +2d0*ptj**2+2d0*ptj*vpt)
      Q_V=dsqrt(mh**2+2d0*ptj**2+2d0*ptj*
...
C --------------------------------------------------------------------------
      SUBROUTINE PNX1X2INT(X1,X2,SH,TH,UH,AMU,PLUMI)
C --------------------------------------------------------------------------
...
C IDENTICAL PARTICLE
      ...
      ELSE  IF(TYPE_V.EQ.'HJ') THEN
       G_G = SPLITSUD(0,A_X_A,AMU)*APDF(0,A_X_B,AMU)*A_X_A
     >     + APDF(0,A_X_A,AMU)*SPLITSUD(0,A_X_B,AMU)*A_X_B
